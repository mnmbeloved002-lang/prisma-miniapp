...
try {
	...
	const changedFiles = run("git diff --name-only HEAD^ HEAD")
		.split("\n")
		.filter(Boolean);
	...
	for (const file of allChangedFiles) {
		...
		if (!isAllowed) {
			...
			console.error(
				`[🛑 FAIL] Нарушение allow-list: Изменён неразрешенный файл: ${file}`,
			);
		}
		...
		if (file === NEWS_FILE_PATH) {
			...
			console.log(
				`[INFO] Diff size для ${file}: +${added} / -${deleted} линий`,
			);
		}
	}
	...
	if (!isOnlyAllowedFiles) {
		throw new Error(
			"PR содержит изменения в неразрешенных файлах. Запрещено смешивание контента и кода/конфигов.",
		);
	}
	...
	if (diffLines > MAX_ALLOWED_DIFF_SIZE) {
		throw new Error(
			`[🛑 FAIL] Превышен лимит изменения контента: ${diffLines} строк (лимит: ${MAX_ALLOWED_DIFF_SIZE}).`,
		);
	}
	...
	const validateOutput = run(
		`node scripts/validate-news.cjs ${NEWS_FILE_PATH}`,
	);
	...
	if (newsCount === 0) {
		throw new Error(
			'[🛑 FAIL] Найден "0 новостей" в финальном news.json. Это ошибка ingestion-пайплайна.',
		);
	}

	console.log(
		`[✅ PASS] Контракт данных соблюден. Найдено ${newsCount} новостей.`,
	);
} catch (error) {
