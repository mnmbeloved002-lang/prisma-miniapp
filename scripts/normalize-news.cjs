...
function normalizeNews(rawNews) {
  ...
  for (const item of rawNews) {
    try {
      ...
      tags = tags
        .map(tag => tag.toString().toLowerCase().replace(/[^\wа-яё]/gi, ''))
  ...
  console.log(`✅ Нормализация: ${normalized.length} успешно, ${skipped} пропущено`);
  return normalized;
}

function saveNormalizedNews(news) {
  const currentNewsPath = path.join(__dirname, '..', 'public', 'news.json');
  const normalizedPath = path.join(__dirname, '..', 'docs', 'fetch', 'normalized-news.json');
...
async function main() {
  try {
    const fetchPath = path.join(__dirname, '..', 'docs', 'fetch', 'last-fetch.json');
