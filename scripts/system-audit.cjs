...
if (fs.existsSync("package.json")) {
	try {
		...
		if (pkg.dependencies) {
			passed.push(
				`✅ dependencies - настроены (${Object.keys(pkg.dependencies).length} пакетов)`,
			);
		}

		if (pkg.devDependencies) {
			passed.push(
				`✅ devDependencies - настроены (${Object.keys(pkg.devDependencies).length} пакетов)`,
			);
		}
	} catch (e) {
...
if (errors.length === 0 && warnings.length === 0) {
	...
	console.log(
		"🟠 ТРЕБУЕТСЯ ВНИМАНИЕ - есть критические ошибки для исправления",
	);
} else {
	console.log(
		"🔴 КРИТИЧЕСКОЕ СОСТОЯНИЕ - требуется срочное исправление ошибок",
	);
}
