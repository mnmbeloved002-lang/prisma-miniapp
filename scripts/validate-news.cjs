  ...
  try {
    ...
    news.forEach((item, index) => {
      ...
      requiredFields.forEach(field => {
        if (!item[field]) {
          errors.push(`❌ Запись ${index}: отсутствует обязательное поле "${field}"`);
        }
      });
      ...
      if (item.date && !item.date.endsWith('Z')) {
        errors.push(`❌ Запись ${index}: дата должна заканчиваться на Z (UTC) - "${item.date}"`);
      }
      
      // Проверка HTTPS
      if (item.url && !item.url.startsWith('https://')) {
        errors.push(`❌ Запись ${index}: URL должен начинаться с https:// - "${item.url}"`);
      }
      
      // Проверка длины title
      if (item.title && (item.title.length < 10 || item.title.length > 180)) {
        errors.push(`❌ Запись ${index}: title должен быть 10-180 символов - ${item.title.length}`);
      }
      ...
      if (item.tags) {
        if (!Array.isArray(item.tags)) {
          ...
          if (item.tags.length < 1 || item.tags.length > 5) {
            errors.push(`❌ Запись ${index}: tags должен содержать 1-5 элементов - ${item.tags.length}`);
          }
          item.tags.forEach(tag => {
            if (tag.length < 2 || tag.length > 24) {
              errors.push(`❌ Запись ${index}: тег "${tag}" должен быть 2-24 символа`);
            }
          });
        }
      }
    ...
    for (let i = 1; i < news.length; i++) {
      ...
      if (prevDate < currDate) {
        errors.push(`❌ Нарушена сортировка: запись ${i-1} (${news[i-1].date}) должна быть после ${i} (${news[i].date})`);
      }
    }
