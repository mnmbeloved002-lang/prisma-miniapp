...
const file = Object.keys(cov).find(p => p.endsWith('src/application/bookmarks.ts'));
if (!file) { console.error('bookmarks.ts not found in coverage'); process.exit(1); }
...
for (const [id, meta] of Object.entries(branchMap)) {
  ...
  if (zeros.length) {
    ...
    if (meta.locations) {
      for (const [k, L] of meta.locations.entries?.() ?? Object.entries(meta.locations)) {
        console.log(`   • arm[${k}] @ ${L.start.line}:${L.start.column}–${L.end.line}:${L.end.column}`);
      }
    }
  }
}
