...
  // --- (НАЧАЛО НОВОГО ТЕСТА ДЛЯ ПОКРЫТИЯ СТР. 19) ---
  /**
   * ЦЕЛЬ: Покрыть строку 19 (Array.isArray(data) ? ... : [])
   * Этот тест проверяет ветку 'false', когда 'data' - не массив (а объект)
   */
  it('should handle corrupted (non-array) data from storage', () => {
    // 1. Симулируем "мусор" (объект вместо массива)
    mockedStorage.get.mockReturnValue({ id: 'corrupted' } as any);
    
    // 2. Вызываем list(), который триггерит ensureLoaded()
    const result = list();
    
    // 3. Ожидаем, что 'ensureLoaded' вернет безопасный пустой массив
