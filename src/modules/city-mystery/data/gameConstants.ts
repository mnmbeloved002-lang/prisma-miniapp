...
export const MOTIVE_CARDS: Record<Motive, MotiveCard> = {
  ...
  VIGILANTE: {
    ...
    condition: (victim, gameState) => {
      ...
      if (victimPos === null || detectivePos === null) {
        return false;
      }
  ...
  PSYCHOPATH: {
    ...
    condition: (victim, gameState) => {
      ...
      if (usedAges.size === 2) {
        return usedAges.has(victim.age);
      }
      return false;
    },
  },
...
export function getAdjacentDistricts(index: number): number[] {
  ...
  if (x > 0) {
    neighbors.push(getDistrictIndex(x - 1, y));
  }
  if (x < 3) {
    neighbors.push(getDistrictIndex(x + 1, y));
  }
  if (y > 0) {
    neighbors.push(getDistrictIndex(x, y - 1));
  }
  if (y < 3) {
    neighbors.push(getDistrictIndex(x, y + 1));
  }
