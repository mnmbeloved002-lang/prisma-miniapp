...
import { getDistrictForResident, getAdjacentDistricts } from './gameConstants';
...
describe('City Mystery Game Rules', () => {
  ...
  test('detective can move to adjacent district', () => {
    let currentState = nextPhase(state).state!; 
  ...
  test('police station gives +1 action', () => {
    ...
    if (!policeBuilding) throw new Error('No police found in setup');
    ...
    const buildingInNewState = result.state!.buildings.find(b => b.type === 'POLICE' && b.position === policeBuilding.position);
  ...
  test('city phase moves citizens without losing them', () => {
    ...
    const gridChanged = JSON.stringify(state.grid) !== JSON.stringify(nextRoundState.grid);
