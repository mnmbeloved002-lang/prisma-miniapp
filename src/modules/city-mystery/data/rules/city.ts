...
import { cloneState, pickRandom, shuffle, type GameRuleResult } from './utils';
...
export function addPopulation(state: GameState): GameRuleResult {
  ...
  for (const citizen of newCitizens) {
    if (availableDistricts.length === 0) break;
 ...
 */
function performFactionMigration(
  grid: Citizen[][], 
  crimeScenes: number[]
): Citizen[][] {
  ...
  for (const i of processOrder) {
    if (newGrid[i].length === 0) continue;
    if (crimeScenes.includes(i)) continue;
...
export function performCityPhase(state: GameState): GameRuleResult {
  ...
  if (currentState.step === 'POPULATION') {
    ...
    if (!popResult.isValid) return popResult;
