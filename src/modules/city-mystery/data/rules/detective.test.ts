...
describe('Detective: Финальное обвинение', () => {
  test('детектив побеждает при правильном обвинении', () => {
    ...
    const result = makeAccusation(state, state.killer.identity.id, state.killer.motive);
  ...
  test('убийца побеждает при обоих неправильных', () => {
    ...
    const result = makeAccusation(state, innocent?.id, wrongMotive!);
  ...
  test('нельзя обвинить несуществующего жителя', () => {
    ...
    const result = makeAccusation(state, 'non-existent-id', state.killer.motive);
