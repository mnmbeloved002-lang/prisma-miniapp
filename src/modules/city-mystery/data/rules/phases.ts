...
import { cloneState, type GameRuleResult, pickRandom } from './utils';
...
export function nextPhase(state: GameState): GameRuleResult {
  ...
  switch (state.phase) {
    ...
    case 'DETECTIVE':
      // Финальное обвинение: блокируем переход по фазам только после завершения раунда

      if (
        state.awaitingFinalAccusation &&
        state.detective.actionsLeft === 0 &&
        state.detective.movementPoints === 0
      ) {
    ...
    case 'CITY':
      ...
      newState.buildings.forEach((b) => {
        b.usedThisRound = false;
      });

      // Финальный раунд после 5-го убийства: доигрываем CITY и переходим к финальному обвинению
      if (newState.awaitingFinalAccusation) {
        newState.phase = 'DETECTIVE';
        newState.step = 'INVESTIGATE';
        newState.detective.actionsLeft = 0;
        newState.detective.movementPoints = 0;
        break;
      }
