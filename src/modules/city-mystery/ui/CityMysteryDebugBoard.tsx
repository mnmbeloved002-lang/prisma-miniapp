...
import { GameState } from '../data/gameEngine';
  ...
  return (
    <div className="mt-4 p-4 bg-black text-green-400 font-mono text-xs rounded border border-green-800">
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          <p>Game ID: <span className="text-white">{gameState.gameId}</span></p>
          <p>Round: <span className="text-white">{gameState.round}</span></p>
          <p>Detective Pos: <span className="text-white">{gameState.detectivePos}</span></p>
        </div>
        <div className="border-l border-green-800 pl-4">
          <p className="text-red-400 font-bold">SECRET INFO:</p>
          <p>Killer ID: <span className="text-white">{gameState.secretIdentity?.killerId}</span></p>
          <p>Motive: <span className="text-white">{gameState.secretIdentity?.motiveId}</span></p>
        </div>
      </div>
    </div>
  );
